...
- 部署页选择服务器和工作内容后，会在一个事务中创建任务、执行计划和首个执行步骤；不再生成执行页不可见的空任务。
- 旧版本留下的空任务会在“执行与变更”中显示，可一键补齐基于原模板和项目资料的执行草案，不用重新创建或手填整张表单。
...
- 桌面端内置一个标准 `stdio` MCP Server，并在侧边栏提供“MCP 接入”页面自动定位数据库、生成配置和执行一键自检。
- 支持查询项目、技术栈、脱敏服务器资产、任务、计划、步骤、人工证据、知识库和审计事件，并可诊断“任务缺计划、计划缺步骤、执行缺证据”等流程断点。
- MCP Server 以只读方式打开本机 SQLite，强制启用 `query_only`；不提供 SSH、Shell、SQL 执行、远程连接或写入工具。
- 接入客户端需要 Node.js 22.13 或更高版本。运行 `npm run mcp:test` 可使用标准 MCP Client 完成协议级联通测试。
